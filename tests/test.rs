...
use futures::future::{err, join_all, lazy, ok, pending, try_join_all};
use futures::prelude::*;
    ...
    C: Default + Send + Sync + 'static,
{
    ...
    fn connect(&self) -> BoxFuture<Result<Self::Connection, Self::Error>> {
        Box::pin(ok(Default::default()))
    }

    fn is_valid(
        &self,
        conn: Self::Connection,
    ) -> BoxFuture<Result<Self::Connection, (Self::Error, Self::Connection)>> {
        Box::pin(ok(conn))
    }
...
// let mut event_loop = Runtime::new().unwrap();
// let pool = event_loop
// .block_on(lazy(|_| Pool::builder().build(Handler)))
// .unwrap();
...
// tx1.send(()).unwrap();
// rx2.then(|r| match r {
// Ok(v) => future::ready(Ok((v, conn))),
// Err(_) => future::ready(Err((Error, conn))),
// })
// })
// .map_err(|_| ()),
// );
...
// assert!(event_loop
// .block_on(async { Timeout::new(pending::<()>(), Duration::from_secs(1)).await })
// .is_err());
